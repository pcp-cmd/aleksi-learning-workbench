...
async function makeFixture(withInstaller = true): Promise<string> {
  ...
  await writeFixtureFile(
    root,
    "package.json",
    `${JSON.stringify({ name: "aleksi-learning-workbench", version: "0.1.2" })}\n`
  );
  await writeFixtureFile(
    root,
    "package-lock.json",
    `${JSON.stringify(
      {
        ...
        lockfileVersion: 3,
        packages: {
          "": { name: "aleksi-learning-workbench", version: "0.1.2" },
          "node_modules/react": {
            version: "19.1.0",
            license: "MIT",
...
describe("release evidence package", () => {
  ...
  it("rejects release evidence when source versions drift", async () => {
    ...
    await expect(
      ...
      /package\.json version is 9\.9\.9; expected canonical version 0\.1\.2/u
    );
  });
