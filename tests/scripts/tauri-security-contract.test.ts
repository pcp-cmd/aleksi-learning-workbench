...
function generatedPermissionSource(command: string): string {
...
describe("Tauri production security contract", () => {
  it("keeps generated command permission files synchronized with the app manifest", async () => {
    const root = process.cwd();
    const [buildScript, generatedPermissions] = await Promise.all([
      readFile(join(root, "src-tauri/build.rs"), "utf8"),
      readdir(join(root, "src-tauri/permissions/autogenerated"))
    ]);
    const commandBlock = buildScript.match(/\.commands\(&\[([\s\S]*?)\]\)/u)?.[1];
    const manifestCommands = [...(commandBlock?.matchAll(/"([a-z_]+)"/gu) ?? [])]
      .map((match) => match[1])
      .sort();
    const expectedPermissions = manifestCommands
      .map((command) => `${command}.toml`)
      .sort();

    expect(manifestCommands).toEqual([...COMMANDS].sort());
    expect(generatedPermissions.sort()).toEqual(expectedPermissions);
    for (const command of manifestCommands) {
      ...
      ).resolves.toBe(generatedPermissionSource(command));
    }
  });
