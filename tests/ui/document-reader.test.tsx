...
describe("document Markdown navigation", () => {
  it("intercepts a handled local reading link without treating it as a browser URL", async () => {
    ...
    const oneChunk = descriptor();
    oneChunk.chunks = oneChunk.chunks.slice(0, 1);
    oneChunk.outline = oneChunk.outline.slice(0, 1);
    ...
    const link = await screen.findByRole("link", { name: "本节实验" });
