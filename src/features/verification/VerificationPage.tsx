...
type AssistanceLevel = "none" | "hint" | "source" | "ai";
type EvidenceStatus = "awaiting-verification" | "accepted" | "repair-needed" | "revoked" | "affected";
type RelationType = "requires" | "proves_with" | "illustrates" | "refutes" | "replaces";
type VerifierKind = "ai-review" | "human-review" | "gpt-plus-import";
type Finding = { location: string; issue: string };
...
type EvidenceSummary = {
  ...
  relations: Array<{ targetEvidenceId: string; targetCardId: string; type: RelationType }>;
  ...
  contextSnapshot: null | {
    ...
    sourceSnapshots: Array<{ readingId: string; relativePath: string; snapshotSha256: string; excerpt: string; locator: string | null }>;
  };
  revocationImpacts: Array<{ rootEvidenceId: string; evidenceId: string; upstreamEvidenceId: string | null; path: string[]; reason: string; revokedAt: string }>;
};
...
export function VerificationPage() {
  ...
  const parseGptJson = () => {
    try {
      const parsed: unknown = JSON.parse(gptJson);
      if (
      ...
      setSummary(parsed.verificationReport.summary);
      setCriticalErrors(parsed.verificationReport.criticalErrors.length > 0 ? parsed.verificationReport.criticalErrors : [emptyFinding()]);
      setGaps(parsed.verificationReport.gaps.length > 0 ? parsed.verificationReport.gaps : [emptyFinding()]);
